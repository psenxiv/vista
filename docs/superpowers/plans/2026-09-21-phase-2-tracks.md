...
is "author a shot, play it back" (`spec:517-520`).

The editor is **not** in this phase; it is phase 2c (`spec:522-523`). Authoring happens through
a simple test window standing in for the editor. It is scaffolding for testing, not
product surface — the real flow is a button and a hotkey (`spec:371-373`), and a curve
editor (`spec:383-386`).
...
test named is one the spec's list asks for (`spec:463-478`). I verify all of part A myself
on macOS before you see any of it.

### A1 — track model
`ControlPoint`, `Track`, `TimingKey`, `TangentMode`, `SnapPoint`, `AimMode`, `PlaybackMode` as
`spec:156-161` and `spec:231-235`. `AimMode` is `PathTangent` or `AimKeys` (`spec:196-203`);
there is no LookAt target. Per-point FoV (`spec:163`). `SnapPoint` stays distinct so
degenerate cases never reach the spline (`spec:296-299`).

`feat(core) add the track model`

### A2 — centripetal Catmull-Rom
Alpha 0.5 (`spec:169`). The curve passes through every control point (`spec:171-173`).
Endpoints duplicate for phantom points, and the path is always open (`spec:179-181`). Two
points is a straight dolly (`spec:179-180`). Coincident neighbours give a zero knot interval,
...
fraction of that segment's arc length — to a spline parameter (`spec:189-192`). Rebuilds on
edit, not per frame.

Test: a bunched-then-spread track yields even spacing (`spec:464-465`).
...
points (`spec:468-469`); a flat section holds the camera still (`spec:470`); the curve never
decreases, including for keys a naive cubic would overshoot (`spec:471-472`).
...
path, never earlier frames, so scrubbing and playback agree (`spec:198-202`).

AimKeys on separate yaw and pitch channels, never slerped (`spec:203-207`). Yaw unwraps
before interpolation (`spec:209-211`). Yaw, pitch
...
Test: yaw crossing ±180° takes the short way (`spec:466`).
...
same function in 2c (`spec:379-381`). Elapsed is a `double`.
...
and fifty times (`spec:467`); a finished `Once` track holds its last frame (`spec:477`); a
`Loop` track cuts back to its first frame after its last key (`spec:474`).
...
(`spec:384-386`). Each control point gets a key at its own position; a hold adds a second key
...
Test: `Tick` returns null whenever live mode is off (`spec:478`).
...
Director. The plugin is in one of three modes, and never two at once (`spec:84-89`):
...
position without writing `Distance` (`spec:103-106`, `spec:129-132`).
...
turn live mode off and stop playback, not just drop the camera (`spec:343-350`).
...
- **Capture point** appends the current camera as a control point (`spec:367-369`).
...
(`spec:357-358`). The `/ccam fly` command goes; the phase-1 debug commands (`hold`, `push`,
...
- **Default aim mode: AimKeys.** Capture records yaw and pitch (`spec:367-369`), so
  replaying what you looked at is the natural default for a track built by flying.
- **Pitch clamp for PathTangent: ±89°** (`spec:198-199` asks for a clamp, no number).
...
- **A track with no points: `Tick` returns null.** The spec requires only that it not throw
  (`spec:475`); null follows from "null means hands off".
...
| | Why |
|---|---|
| Curve editor — draggable keys and tangent handles | phase 2c (`spec:522-523`). Until then the curve is generated, not shaped |
| The rest of the editor UI — windows, scrub bar, 3D overlay, click-to-select, ImGuizmo | phase 2c; the gizmo convention check is its first task (`spec:416-421`) |
| Switchboard: slots, program/preview, TAKE, hotkeys | phase 3 (`spec:525-526`) |
| Persistence, and its config round-trip test | phase 3 (`spec:525-526`) |
...
- **The monotonicity limiter is the one subtle bit.** Get it wrong and the camera judders
  backwards at a key. It has a dedicated test (`spec:471-472`).
