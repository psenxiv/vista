...
is "author a shot, play it back" (`spec:535-538`).

The editor is **not** in this phase; it is phase 2c (`spec:540-541`). Authoring happens through
a simple test window standing in for the editor. It is scaffolding for testing, not
product surface — the real flow is a button and a hotkey (`spec:389-391`), and a curve
editor (`spec:401-404`).

## Timing model

**Path and timing are separate** (`spec:231-235`). The control points and the spline through
them describe geometry only. A separate **timing curve** — position along the path, plotted
against time — carries every pacing decision (`spec:253-258`):
...
is how far along that segment's arc length (`spec:243-247`). Keys are anchored to points, so
moving or appending a point never retimes a shot (`spec:249-251`).

`Auto` tangents make a smooth pass-through the default (`spec:260-264`). At the ends they are
one-sided: the camera starts at full speed and stops dead. Easing is the user's choice, via a
`Flat` key (`spec:266-267`).

Looping is a **playback mode**, not a path shape: `Loop` hard-cuts from the last key back to
the first (`spec:280-285`). The path is always open. Future modes change only how elapsed time
...
test named is one the spec's list asks for (`spec:481-496`). I verify all of part A myself
on macOS before you see any of it.

### A1 — track model
`ControlPoint`, `Track`, `TimingKey`, `TangentMode`, `SnapPoint`, `AimMode`, `PlaybackMode` as
`spec:162-167` and `spec:237-241`. `AimMode` is `PathTangent` or `AimKeys` (`spec:202-209`);
there is no LookAt target. Per-point FoV (`spec:169`). `SnapPoint` stays distinct so
degenerate cases never reach the spline (`spec:302-305`).
...
Test: a bunched-then-spread track yields even spacing (`spec:482-483`).
...
the curve never decreases (`spec:269-272`) — a naive cubic overshoots and the camera
reverses. `Flat` pins tangents to zero. `Linear` and `Manual` are defined but unreachable
until the curve editor (`spec:263-264`).
...
dead stop at the last (`spec:266-267`). Ends hold their value (`spec:274-276`), which is
where "a finished track holds its final frame" comes from rather than a special case. Total
length is the last key's time (`spec:278`). The curve knows nothing about playback modes;
looping is A6's job (`spec:280-285`).

Tests: a straight curve gives constant world speed within a segment on unevenly spaced
points (`spec:486-487`); a flat section holds the camera still (`spec:488`); the curve never
decreases, including for keys a naive cubic would overshoot (`spec:489-490`).
...
during a hold without special handling (`spec:225-227`). Yaw and pitch become a look-at
through `FreeCamMotion.LookAtFrom`, so a captured aim replays exactly as it was flown.

Test: yaw crossing ±180° takes the short way (`spec:484`).
...
same at 30 and 144fps (`spec:287-288`).

Evaluation is a pure `Evaluate(track, time)`, which `Tick` calls; the scrub bar needs the
same function in 2c (`spec:397-399`). Elapsed is a `double`.

The playback mode lives here and only here (`spec:280-285`). `Once` clamps elapsed at the
...
and fifty times (`spec:485`); a finished `Once` track holds its last frame (`spec:495`); a
`Loop` track cuts back to its first frame after its last key (`spec:492`).
...
(`spec:402-404`). Each control point gets a key at its own position; a hold adds a second key
...
### A8 — Director
`CameraState? Tick(float dt)` (`spec:292-294`). Non-null means we own the camera, null means
hands off (`spec:296-300`). Live mode off returns null. There is one live mode; playing a
track is live mode with that track on program (`spec:313-317`). A shot is a Track, a
SnapPoint, or GameCamera (`spec:302`).

The Director can pause: live stays on and `Tick` keeps returning the frame it stopped on.

Test: `Tick` returns null whenever live mode is off (`spec:496`).
...
turn live mode off and stop playback, not just drop the camera (`spec:346-353`).
...
- **Capture point** appends the current camera as a control point (`spec:385-387`).
...
paused included (`spec:318`), and while off. Invalid leg or hold input is rejected by A7's
validation and shown in the window, not applied.

Commands: `/ccam` opens the window. `/ccam release` stays as a text escape route
(`spec:360-361`). The `/ccam fly` command goes; the phase-1 debug commands (`hold`, `push`,
...
- **Default aim mode: AimKeys.** Capture records yaw and pitch (`spec:385-387`), so
...
- **A track with no points: `Tick` returns null.** The spec requires only that it not throw
  (`spec:493`); null follows from "null means hands off".
...
- **Auto-tangent limiter: Fritsch–Carlson.** The spec requires monotonicity (`spec:269-272`)
...
| | Why |
|---|---|
| Curve editor — draggable keys and tangent handles | phase 2c (`spec:540-541`). Until then the curve is generated, not shaped |
| The rest of the editor UI — windows, scrub bar, 3D overlay, click-to-select, ImGuizmo | phase 2c; the gizmo convention check is its first task (`spec:434-439`) |
| Switchboard: slots, program/preview, TAKE, hotkeys | phase 3 (`spec:543-544`) |
| Persistence, and its config round-trip test | phase 3 (`spec:543-544`) |
...
- **The monotonicity limiter is the one subtle bit.** Get it wrong and the camera judders
  backwards at a key. It has a dedicated test (`spec:489-490`).
