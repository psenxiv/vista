...
public static class TimingCompiler
{
    ...
    {
        if (track.Timing.Count != track.Points.Count)
            throw new ArgumentException("every point needs one timing entry");
        ...
        for (var p = 0; p < track.Points.Count; p++)
        {
            ...
            if (p > 0)
                time += LegDuration(legLengths[p], t.LegSpeed ?? track.Speed);
            ...
            keys.Add(
                new TimingKey(
                    ...
                    p,
                    t.InMode,
                    holds ? TangentMode.Auto : t.OutMode,
                    t.InTangent,
                    holds ? 0f : t.OutTangent,
                    t.Broken
                )
            );
            if (!holds)
