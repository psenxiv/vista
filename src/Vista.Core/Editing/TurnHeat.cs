...
public static class TurnHeat
{
    ...
    /// <summary>Degrees per second drawn fully hot; half of it is warm.</summary>
    ...
    /// <summary>The camera's place and turn rate every 1/<see cref="SamplesPerSecond"/> seconds through the track, aimed at <paramref name="target"/> when given; the first sample takes the second's rate.</summary>
    public static IReadOnlyList<Sample> Samples(TrackEvaluator evaluator, Vector3? target = null)
    {
        ...
        if (count > 1) samples[0] = samples[0] with { DegreesPerSecond = samples[1].DegreesPerSecond };
        return samples;
    }

    /// <summary>Where <paramref name="degreesPerSecond"/> falls on the scale: 0 at rest, 0.5 warm, 1 at <see cref="Hot"/> and above.</summary>
