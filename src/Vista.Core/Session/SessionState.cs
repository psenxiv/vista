...
public enum EditOutcome { Unchanged, FromGame, FromLive }

/// <summary>What <see cref="SessionState.Play"/>, <see cref="SessionState.Restart"/> or <see cref="SessionState.Cue"/> did.</summary>
public enum PlayOutcome { Refused, ReHid, Resumed, Started, StartedFromGame, Cued, CuedFromGame, Previewed }
...
public sealed class SessionState
{
    ...
    public bool IsPlaying => Transport.Previewing || (Mode == CameraMode.Live && !Director.IsPaused && !Director.IsFinished);
    ...
    public EditOutcome Edit()
    {
        ...
        switch (Mode)
        {
            ...
            case CameraMode.Live:
                Transport.DropScrub();
                Transport.Park(PlayingEntry?.TrackId == EditedTrackId ? Math.Clamp(Director.ShotTime, 0.0, Duration) : 0.0);
                Director.GoOffline();
    ...
    {
        if (Mode == CameraMode.Editing) return Transport.Previewing ? PlayOutcome.Previewed : StartPreview(fromStart: false);
    ...
    public bool Stop()
    {
        if (Mode == CameraMode.Editing) return Transport.StopPreview();
    ...
    {
        if (to is not (CameraMode.Off or CameraMode.View)) throw new ArgumentOutOfRangeException(nameof(to), to, "Release goes to Off or View.");
        Transport.StopPreview();
        var owned = !Released;
        if (owned)
        {
    ...
    public string? SelectPoint(Guid track, int index)
    {
        ...
        if (refusal is null) Selection.Select(index);
        return refusal;
    }
    ...
    public string? AddAfterSelected(ControlPoint point)
    {
        ...
        if (added is null) Selection.LastPoint = s + 1;
        return added;
    }
    ...
    public string? DeletePoints(IReadOnlyCollection<int> indices)
    {
        ...
        if (refusal is null) Selection.LastPoint = last;
        return refusal;
    }
    ...
    private ControlPoint ToLocal(ControlPoint world) => World.FollowFrame(Local) is { } frame ? frame.ToLocal(world) : SceneGeometry.WorldAnchor(Scene, Local).ToLocal(world);
    ...
    private Scene WithPoint(Scene scene, ControlPoint world, Func<Track, ControlPoint, Track> add)
    {
        ...
        if (local.Aim == AimMode.FollowTarget)
        {
            ...
            if (World.FollowFrame(scene, local) is null) throw new ArgumentException("Character not found");
        }
        ...
        var stored = World.FollowFrame(placed, track) is { } frame ? frame.ToLocal(world) : SceneGeometry.WorldAnchor(placed, track).ToLocal(world);
    ...
    public string? RemoveHold(int key)
    {
        ...
        if (refusal is null) Selection.Key = null;
        return refusal;
    }
    ...
    public string? SetAim(AimMode aim, ControlPoint camera)
    {
        ...
        return ApplySetting(t =>
        {
            ...
            return shown is { } p && World.FollowFrame(Scene, result) is { } frame ? TrackEditing.Replace(result, 0, frame.ToLocal(p)) : result;
        });
    }
    ...
    public string? SetTarget(string? name, string? world)
    {
        ...
        return ApplySetting(t =>
        {
            ...
            return !ReferenceEquals(result, t) && t.TargetName is null && shown is { } s && World.FollowFrame(Scene, result) is { } frame ? TrackEditing.Replace(result, 0, frame.ToLocal(s)) : result;
        });
    }
    ...
    public string? PreviewAnchor(Anchor world, bool carry)
    {
        ...
        if (Selection.Anchor is not { } kind || kind == AnchorKind.LookAt) return "Select an anchor first.";
        if (Selection.UnplacedRefusal(kind) is { } unplaced) return unplaced;
        Scene = kind == AnchorKind.Scene
    ...
    {
        if (liveEditStart is null) return "No live edit is in progress.";
        if (Selection.Anchor != AnchorKind.LookAt) return "Select the Look At point first.";
        if (Selection.UnplacedRefusal(AnchorKind.LookAt) is { } unused) return unused;
    ...
    private string? ApplyScene(Func<Scene, Scene> change, Func<Track, IReadOnlyList<int>> selectAfter)
    {
        ...
        if (refusal is null) Selection.RefreshTiming(pointsBefore);
        return refusal;
    }
