...
internal sealed class CameraWindow : Window
{
    ...
    public CameraWindow(GameSession game) : base("Camera###vista-camera")
    {
        this.game = game;
    ...
    public override void Draw()
    {
        ...
        ImGui.TableNextRow();
        if (PoseGrid.Button("level-roll", FontAwesomeIcon.SyncAlt, "Level roll", enabled: true)) game.CameraRoll = 0f;
        ...
        Field("cam-roll", "Roll", EditorColours.AxisZ, PoseGrid.Degrees(game.CameraRoll), PoseGrid.AngleSpeed, "%.1f°",
        ...
        if (PoseGrid.Button("reset-fov", FontAwesomeIcon.History, "Reset to the game's field of view", enabled: takeover is not null) && takeover is { } fov)
            game.CameraFov = fov;
        Field("cam-fov", "FoV", null, PoseGrid.Degrees(game.CameraFov), PoseGrid.FovSpeed, "%.1f°",
    ...
    private void DrawNudges(Vector3 position, float yaw, float pitch)
    {
        ...
        void Nudge(string id, string name, uint border, Func<float, Vector3> input)
            => Field(id, name, border, 0f, PoseGrid.PositionSpeed, "%.2f",
                v => { if (float.IsFinite(v)) game.CameraPosition = FreeCamMotion.Step(position, input(v), yaw, pitch, 1f, 1f); });
