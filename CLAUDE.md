...
**Mutation testing is slow: it runs once per piece of work, by whoever does the final review, never by an implementer or a fixer.** `make soak` is slow too: an implementer runs `make verify`, then `make soak` once just before committing, never repeatedly while iterating; a fixer or reviewer runs `make verify` only and never re-runs soak. The final review re-runs `make soak` only if Core changed since the last soak, then runs `make mutate SINCE=<base commit>` once over all the changed Core files. Each survivor gets a test or a line in the plan saying why it changes nothing. There's no score to reach.
